...
// 自定义命令行参数帮助信息
func customUsage() {
	fmt.Fprintf(os.Stdout, `slow-log-summary version: 1.0.0
Usage:
...
func (c *Config) ParseFlags() {
	resultFileName := fmt.Sprintf("/tmp/slow-log-summary-%s.html", currentTime.Format("20060102-150405"))
...
func getSlowLogSummaryByPtQueryDigest(ptQueryDigestCmd []string, slowlogFile string, now string) map[string]interface{} {
	cmdTimeWithQuotes := make([]string, len(ptQueryDigestCmd))
	for i, arg := range ptQueryDigestCmd {
		if strings.Contains(arg, " ") {
			cmdTimeWithQuotes[i] = `"` + arg + `"`
		} else {
			cmdTimeWithQuotes[i] = arg
		}
	}
	fmt.Printf("Executing pt-query-digest command: %s\n", strings.Join(cmdTimeWithQuotes, " "))
